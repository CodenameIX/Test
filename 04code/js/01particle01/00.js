...
function createParticles() {
	var x;
	for (x = 1; x < _canvasWidth - 10; x += 25) {
		_particles.push(new Particle(x - _canvasHalfWidth, -_canvasHalfHeight + (Math.random() * _canvasHeight)));
	}
...
function redraw() {
	...
	if (_particles.length < MAX_PARTICLES) {
		...
		particle = new Particle(_canvasHalfWidth,_canvasHalfHeight);
		...
		for (j = i + 1; j < _particles.length; j++) {
			...
			if (distance < RADIUS) {
				...
				if (lines <= MAX_LINES) {
					...
					opacity = 0.9 * Math.min((1 - distance / RADIUS), particleA.getOpacity(), particleB.getOpacity());
					_context.beginPath();
